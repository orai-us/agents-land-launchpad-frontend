...
const PreSaleModal: React.FC<ModalProps> = ({ isOpen, closeModal, quantity, setQuantity, onConfirm }) => {
  return (
    <div className={twMerge('fixed w-full inset-0 flex items-center justify-center z-50 backdrop-blur-md', !isOpen && 'hidden')}>
      <div className="flex w-full max-w-[480px] sm:max-w-xl flex-col p-6 rounded-lg gap-3 bg-[#13141D] relative">
        <div className="">
          <h2 className="text-[18px] font-medium">Select quantity of Token (Optional)</h2>
          ...
          <div className="w-full bg-[#080A14] border border-[#1A1C28] rounded-lg p-4 text-[14px] font-medium mt-8">
            ...
            <span className="text-[#9192A0]">its optional but buying a small amount of tokens helps protect your coin from snipers</span>
          </div>
          <div className="mt-6">
            <label htmlFor="presale" className="uppercase text-[12px] font-medium text-[#9192A0]">
              quantity (Optional)
            </label>
          ...
          >
            <div className="uppercase rounded bg-white px-6 py-2 text-[#080A14]">CREATE TOKEN</div>
          </button>
        </div>
      </div>
    </div>
