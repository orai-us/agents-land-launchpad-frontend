  ...
  return (
    <div className={twMerge('fixed w-full inset-0 flex items-center justify-center z-50 backdrop-blur-md', !isOpen && 'hidden')}>
      <div className="flex w-full max-w-[480px] flex-col p-6 rounded-lg gap-3 bg-[#13141D] relative justify-center items-center">
        <img src={successIcon} alt="successIcon" className="w-10 h-10 rounded-full mt-2" />
        <div className="mt-6 text-[18px] text-[#E8E9EE]">New Token Created!</div>
        <div className="uppercase mt-2 mb-8 text-[14px] text-[#9192A0]">${symbol || '--'}</div>

        <button
          onClick={() => setLocation(!token ? '/' : `/trading/${token.toString()}`)}
        ...
        >
          <div className="uppercase rounded bg-white px-6 py-2 text-[#080A14]">View</div>
        </button>
      </div>
    </div>
  );
