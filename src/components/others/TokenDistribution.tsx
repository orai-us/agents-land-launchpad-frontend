  ...
  return (
    <div className="flex flex-col justify-between pt-4">
      ...
      <div className="w-full flex flex-col gap-2 border border-[#1A1C28] rounded-lg p-6">
        <p className="text-[#E8E9EE] text-[16px] uppercase">King of the hill progress ({kothProgress.toFixed(2)}%)</p>
        <div className="w-full my-2 px-[2px] py-[1px] rounded-[28px] bg-[#1A1C28] border border-solid border-[#30344A]">
          <div className="rounded-[999px] h-2 bg-hill " style={{ width: `${kothProgress}%` }}></div>
        </div>

        <span className="text-[#585A6B] text-[14px]">Dethrone the current king at {formatNumberKMB(kothCoin?.marketcap || 0)} market cap</span>
      </div>
      <div className=" border border-[#1A1C28] rounded-lg p-6 mt-4 mb-14">
        <div className="uppercase text-[16px] text-[#E8E9EE]">Holder Distribution</div>
        <div className="w-full flex flex-row justify-between items-center text-sm pt-3">
          <p className="text-[12px] text-[#585A6B] leading-10 uppercase">Holder</p>
          <p className="text-[12px] text-[#585A6B] leading-10 uppercase">Percentage</p>
        </div>
          ...
          {holders &&
            holders
              ...
              .map((item: any, index: number) => {
                ...
                return (
                  <div key={index} className="w-full flex flex-row justify-between text-[14px] text-[#E8E9EE] pr-2 pb-4 border border-b border-[#1A1C28] border-t-0 border-x-0 hover:brightness-75">
                    <div className="flex flex-row gap-1 items-center">
                      <div className="">{item.slice}</div>
                    </div>
                    <div className="flex flex-col">{Math.floor(item.amount / 10_000_000_000) / 1000}%</div>
                  </div>
                );
              })}
        </div>
