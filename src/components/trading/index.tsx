...
import { formatLargeNumber, formatNumberKMB, numberWithCommas } from '@/utils/format';
...
export default function TradingPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const bondingCurvePercent = (data.lamportReserves || new BN(0)).mul(new BN(100)).div(new BN(BONDING_CURVE_LIMIT)).toNumber();
  ...
  const priceUsd = new BigNumber(isNaN(curPrice) ? 0 : curPrice).multipliedBy(solPrice).toFixed(6);
  ...
  return (
    <div className="w-full flex flex-col mx-auto gap-5">
      ...
      {Number(progress) >= 100 && !isListed && (
        <div className="animate-pulse animate-infinite rounded px-3 py-3 mb-4 bg-[#9FF4CF] text-[#080A14] flex items-center justify-center">Token is listing on Oraidex and Raydium. Please wait a minutes!</div>
      )}

      <div className="w-full flex flex-col xs:flex-row gap-10">
            ...
            <div className="bg-[#1A1C28] rounded-t p-6">
              <div className="mb-4 flex justify-between items-start">
                <div className="flex gap-2 items-center">
                  {typeof imgSrc === 'string' ? <img src={imgSrc} alt="agentAvt" className="w-[60px] h-[60px] rounded-full border border-[#E8E9EE]" /> : <img src={imgSrc} alt="agentAvt" className="w-[60px] h-[60px] rounded-full" />}
                  <div>
                    <p className="text-[#E8E9EE] text-[24px] font-medium">
                      {/* {"Jordan’s Investor Coach"}&nbsp;(${coin.ticker}) */}
                      {coin.name || '--'}&nbsp;(${coin.ticker || '--'})
                    </p>
                    <p className="text-[#84869A] text-[12px] mt-2 font-medium uppercase">CONTRACT: {coin.token}</p>
                  </div>
                </div>
                <div className="flex flex-col justify-between items-end h-full">
                  <div className="text-[12px] text-[#84869A] font-medium uppercase">
                    CREATED BY <span className="text-[12px] text-[#E4775D] underline cursor-pointer">{reduceString(coin.creator?.['wallet'] || '', 4, 4)}</span>
                  </div>
              ...
              <div>
                ...
                <p className={twMerge('mt-1 font-medium text-[14px] text-[#84869A]')}>≈ ${priceUsd}</p>
                {/* <p
                className={twMerge(
        ...
        <div className="w-full max-w-[384px]">
          ...
          <div className="flex flex-col gap-3 border border-[#1A1C28] rounded-lg p-6 mt-4">
            <div className="w-full flex flex-col gap-2">
              <p className="text-[#E8E9EE] text-[16px] uppercase">Bonding curve ({progress.toFixed(2)}%)</p>
              <div className="w-full mt-2 px-[2px] py-[1px] rounded-[28px] bg-[#1A1C28] border border-solid border-[#30344A]">
                <div className="rounded-[999px] h-2 bg-barrie" style={{ width: `${progress}%` }}></div>
              </div>
            </div>
            <p className="text-[14px] text-[#585A6B]">
              There are <span className="text-[#E8E9EE]">{formatLargeNumber(fromBig(coin.tokenReserves, coin.decimals))}</span> tokens still available for sale in the bonding curve and there is{' '}
              <span className="text-[#E8E9EE]">{formatLargeNumber(fromBig(coin.lamportReserves, 9))}</span> SOL in the bonding curve.
            </p>
            <p className="text-[14px] text-[#585A6B]">
              When the market cap reaches <span className="text-[#E8E9EE]">{formatNumberKMB(new BigNumber(BONDING_CURVE_LIMIT).multipliedBy(solPrice).div(LAMPORTS_PER_SOL).toNumber())}</span>
              &nbsp; all the liquidity from the bonding curve will be deposited into Raydium and burned. progression increases as the price goes up.
            </p>
          </div>
