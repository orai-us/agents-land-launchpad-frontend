...
const RecentToken: FC<Partial<RecentTokenAction>> = ({ address, type, amount, token, time }) => {
  return type === ACTION_TYPE.Created ? <RecentTokenCreated address={address} type={type} token={token} time={time} /> : <RecentTokenSwap address={address} type={type} token={token} amount={amount} />;
};

const RecentTokenCreated: FC<Pick<RecentTokenAction, 'address' | 'type' | 'time' | 'token'>> = ({ address, type, token, time }) => {
  return (
    <div className="flex items-center px-6 border-r border-[rgba(252,_252,_252,_0.24)] text-[#FCFCFC] text-nowrap">
      ...
      &nbsp;
      {token.name}
      {typeof token.img === 'string' ? <img src={token.img} alt="tokenIMG" className="w-5 h-5 rounded-full ml-2 mr-2" /> : <img src={token.img} alt="tokenIMG" className="w-5 h-5 rounded-full ml-2" />}
      &nbsp;on&nbsp;
...
const RecentTokenSwap: FC<Pick<RecentTokenAction, 'address' | 'type' | 'amount' | 'token'>> = ({ address, type, amount, token }) => {
  return (
    <div className="flex px-6 border-r border-[rgba(252,_252,_252,_0.24)] text-[#FCFCFC] text-nowrap">
      <span className="text-[#FCFCFC]">{address}</span>&nbsp;
      <span className={twMerge('text-[#9FF4CF]', type === ACTION_TYPE.Sold && 'text-[#E75787]')}>{type}</span> &nbsp;
      {amount} SOL of {token.name}
      {typeof token.img === 'string' ? <img src={token.img} alt="tokenIMG" className="w-5 h-5 rounded-full ml-2 mr-4" /> : <img src={token.img} alt="tokenIMG" className="w-5 h-5 rounded-full ml-2 mr-4" />}
    </div>
  );
};
  ...
  useEffect(() => {
    ...
    listener.setProgramLogsCallback('Launch', (basicTokenInfo: any) => {
      ...
      setListNotifications((prevList) => {
        const newItem = {
          address: newCoinInfo.creator['name'] || reduceString(new PublicKey(newCoinInfo.creator).toString(), 4, 4),
    ...
    listener.setProgramLogsCallback('Swap', (swapInfo: SwapInfo) => {
      ...
      setListNotifications((prevList) => {
        const newItem = {
          ...
          amount: ((swapInfo.solAmountInLamports || new BN(0)).toNumber() / LAMPORTS_PER_SOL).toFixed(9),
          token: {
  ...
  return (
    <div className="flex items-center justify-center w-full bg-[#3D2426] p-4">
      <div className="flex items-center justify-center max-w-[1216px] w-full">
        ...
        <Marquee pauseOnClick pauseOnHover gradient gradientColor="#3D2426" gradientWidth={50} speed={150}>
          {/* <div className="w-full flex items-center overflow-x-auto no-scrollbar"> */}
          {notificationList.map((item, idx) => {
            return (
              <div key={`id-noti-${item.address}-${idx}-${item.token.name}`} className="flex cursor-pointer">
                <RecentToken {...item} />
              </div>
            );
          })}
