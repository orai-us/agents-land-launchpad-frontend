...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { uniqueTokenCount, tokenDetails } = await new Web3SolanaProgramInteraction().getNumberOfOwnedToken(publicKey);
    ...
    <div className="w-full h-full flex items-start gap-8 px-2 mt-16">
      <div className="w-full max-w-[350px] bg-[#13141D] p-6 rounded-lg">
        <div className="flex flex-col gap-6">
          <img src={userData.avatar || defaultUserImg} alt="Avatar" className="object-cover w-16 h-16 rounded-lg" />
          <div className="w-full flex flex-col text-white font-medium gap-2">
            ...
            <div className="flex items-center">
              <p className="mr-2 text-[14px] text-[#9192A0]">{reduceString(userData?.wallet || '', 4, 4)}</p>
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none" className="cursor-pointer hover:brightness-125" onClick={() => copyToClipboard(userData?.wallet)}>
                <path
        ...
        <div className="w-full flex flex-row pt-6 items-center justify-between">
          <div className="w-fit flex flex-col justify-center gap-1 text-center">
            <div className="text-[12px] text-[#9192A0] uppercase">TOKEN HELD</div>
            <div className="text-[16px] text-white font-medium">{ownedToken.uniqueTokenCount || 0}</div>
          </div>
          <div className="w-fit flex flex-col justify-center gap-1 text-center">
            <div className="text-[12px] text-[#9192A0] uppercase">followers</div>
            <div className="text-[16px] text-white font-medium">--</div>
          </div>
          <div className="w-fit flex flex-col justify-center gap-1 text-center">
            <div className="text-[12px] text-[#9192A0] uppercase">Following</div>
            <div className="text-[16px] text-white font-medium">--</div>
          </div>
        </div>
        <div className="cursor-pointer bg-[#1A1C28] rounded-lg h-12 flex items-center justify-center mt-6" onClick={() => setProfileEditModal(true)}>
          EDIT PROFILE
        </div>
      </div>
        ...
        <div className="flex">
          {ProfileMenuList.map((item) => (
            <div
              ...
              className={twMerge('cursor-pointer uppercase mr-4 px-4 py-[6px] rounded border border-[rgba(88,_90,_107,_0.32)] text-[#585A6B]', option === item.id && 'bg-[#585A6B] text-[#E8E9EE]')}
            >
              {item.text}
            </div>
          ))}
            ...
            <div className="flex flex-wrap">
              {!coins.length ? (
                <div className="w-full mt-4 rounded-lg bg-[#13141D] border border-dashed border-[#30344A] py-12 px-8 flex flex-col justify-center items-center">
                  ...
                  <p className="mt-2 text-[#585A6B] text-[14px]">No Token Held</p>
                </div>
              ) : (
                  ...
                  <table className="w-full h-full scroll-table">
                    <thead className="w-full text-white border-b-[1px] border-b-[#1A1C28]">
                      <tr className="text-lg">
                        <th className="py-2 text-[#585A6B] text-[12px] uppercase text-left">Token</th>
                        {/* <th className="py-2 text-[#585A6B] text-[12px] uppercase text-right">
                        Price
                      </th> */}
                        <th className="py-2 text-[#585A6B] text-[12px] uppercase text-right">Marketcap</th>
                        {/* <th className="py-2 text-[#585A6B] text-[12px] uppercase text-right">
                      CHANGE
                      </th> */}
                        <th className="py-2 text-[#585A6B] text-[12px] uppercase text-right pr-4">Date Created</th>
                      </tr>
                    </thead>
                    <tbody className="">
                      {coins.map((coin, index) => (
                        <tr key={`${index}-tk-details`} className="w-full border-b-[1px] border-b-[#1A1C28] text-[#E8E9EE]">
                          <td className="flex flex-row gap-2 items-center py-4">
                            <div className="text-lg flex items-center">
                              <img src={coin.url} alt="coinUrl" className="rounded-full w-6 h-6 border border-[#E8E9EE]" />
                              &nbsp;
                              {coin.ticker}
                            </div>
                          </td>
                          ...
                          <td className="py-2 text-right"> {formatNumberKMB(Number(coin.marketcap || 0))}</td>
                          {/* <td className="py-2 text-right">
                            {numberWithCommas(coin.balance || 0)}
                          </td> */}
                          <td className="py-2 text-right">{dayjs(coin.date || Date.now()).format('DD/MM/YYYY')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {!ownedToken.tokenDetails.length ? (
                ...
                <div className="w-full h-full py-4 px-4 border-[#1A1C28] border rounded-lg mt-4">
                  <table className="w-full h-full scroll-table">
                    <thead className="w-full text-white border-b-[1px] border-b-[#1A1C28]">
                      <tr className="text-lg">
                        <th className="py-2 text-[#585A6B] text-[12px] uppercase text-left">Token Address</th>
                        <th className="py-2 text-[#585A6B] text-[12px] uppercase text-right pr-4">Balance</th>
                      </tr>
                    </thead>
                    <tbody className="">
                      {ownedToken.tokenDetails
                        ...
                        .map((tk, index) => (
                          <tr key={`${index}-tk-details`} className="w-full border-b-[1px] border-b-[#1A1C28] text-[#E8E9EE]">
                            <td className="flex flex-row gap-2 items-center py-4">
                              <a href={`https://solscan.io/account/${tk.mint}`} target="_blank" className="text-lg underline hover:cursor-pointer">
                                {reduceString(tk.mint || '', 4, 4)}
                              </a>
                            </td>
                            <td className="py-2 text-right">{numberWithCommas(tk.balance || 0)}</td>
                          </tr>
                        ))}
                    </tbody>
                  </table>
