...
export class PoolKeys {
  ...
  static async fetchPoolKeyInfo(
    ...
    const marketId = await this.fetchMarketId(
      connection,
      baseMint,
      quoteMint,
      "confirmed"
    ...
    const baseMintInfo = (await connection.getParsedAccountInfo(
      baseMint,
      "confirmed"
    )) as MintInfo;
    const baseDecimals = baseMintInfo.value.data.parsed.info.decimals;

    const V4PoolInfo = await this.generateV4PoolInfo(
      baseMint,
      quoteMint,
      marketId
    );
    const lpMintInfo = (await connection.getParsedAccountInfo(
      V4PoolInfo.poolInfo.lpMint,
      "confirmed"
    )) as MintInfo;
    ...
    return {
      ...
      version: 4,
      programId: this.RAYDIUM_POOL_V4_PROGRAM_ID,
      ...
      marketVersion: 3,
      marketProgramId: this.OPENBOOK_ADDRESS,
      marketAuthority: Market.getAssociatedAuthority({
        programId: this.OPENBOOK_ADDRESS,
        marketId: marketId,
      }).publicKey,
